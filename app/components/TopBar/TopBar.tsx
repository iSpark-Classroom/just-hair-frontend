...
const TopBar = ({ user = "Guest" }: { user: string }) => {
  ...
  return (
    <div className="flex items-center justify-center md:justify-between p-6 w-full bg-white h-20 border-b border-neutral-200">
      ...
      {signOut && (
        <div className="absolute top-0 left-0 bg-[#130d1cd0] h-dvh w-dvw flex justify-center items-center z-1000">
          <div className="bg-neutral-50 p-6 flex flex-col items-center justify-center gap-6 rounded-xl">
            ...
            <div className="flex gap-12">
              <Link
                href="/sign-in"
                className="flex gap-2 items-center justify-center w-fit text-red-700 hover:scale-[1.05] font-bold py-3 px-5 rounded-md cursor-pointer transition duration-400 ease-in-out"
              >
                Yes, exit
              </Link>
              <button
